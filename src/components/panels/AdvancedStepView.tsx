import {
  getAdvancedStepById,
  getAdvancedPrevNext,
} from '../../data/advancedSteps'
import { GlassCard } from '../GlassCard'

type AdvancedStepViewProps = {
  stepId: string
  onNavigateToStep: (stepId: string | null) => void
}

export function AdvancedStepView({
  stepId,
  onNavigateToStep,
}: AdvancedStepViewProps) {
  const resolved = getAdvancedStepById(stepId)
  const { prev, next } = getAdvancedPrevNext(stepId)

  if (!resolved) {
    return (
      <section
        id="panel-advanced-step"
        role="tabpanel"
