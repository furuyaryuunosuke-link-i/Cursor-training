import {
  getPythonStepById,
  getPythonPrevNext,
} from '../../data/pythonSteps'
import { GlassCard } from '../GlassCard'
import { StepNavBar } from '../StepNavBar'

type PythonStepViewProps = {
  stepId: string
  onNavigateToStep: (stepId: string | null) => void
}

export function PythonStepView({
  stepId,
  onNavigateToStep,
}: PythonStepViewProps) {
  const resolved = getPythonStepById(stepId)
  const { prev, next } = getPythonPrevNext(stepId)

  if (!resolved) {
    return (
      <section
        id="panel-python-step"
        role="tabpanel"
