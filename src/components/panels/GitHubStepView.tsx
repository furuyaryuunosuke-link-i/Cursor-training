import {
  getGitHubStepById,
  getGitHubPrevNext,
} from '../../data/githubSteps'
import { GlassCard } from '../GlassCard'
import { StepNavBar } from '../StepNavBar'

type GitHubStepViewProps = {
  stepId: string
  onNavigateToStep: (stepId: string | null) => void
}

export function GitHubStepView({
  stepId,
  onNavigateToStep,
}: GitHubStepViewProps) {
  const resolved = getGitHubStepById(stepId)
  const { prev, next } = getGitHubPrevNext(stepId)

  if (!resolved) {
    return (
      <section
        id="panel-github-step"
        role="tabpanel"
