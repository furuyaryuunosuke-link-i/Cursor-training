import {
  getNodeStepById,
  getNodePrevNext,
} from '../../data/nodejsSteps'
import { GlassCard } from '../GlassCard'
import { StepNavBar } from '../StepNavBar'

type NodeStepViewProps = {
  stepId: string
  onNavigateToStep: (stepId: string | null) => void
}

export function NodeStepView({
  stepId,
  onNavigateToStep,
}: NodeStepViewProps) {
  const resolved = getNodeStepById(stepId)
  const { prev, next } = getNodePrevNext(stepId)

  if (!resolved) {
    return (
      <section
        id="panel-nodejs-step"
        role="tabpanel"
        aria-labelledby="tab-nodejs"
        className="space-y-6"
      >
        <GlassCard>
