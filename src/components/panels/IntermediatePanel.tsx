...
export function IntermediatePanel({ onStepSelect }: IntermediatePanelProps) {
  ...
  const handleToggleChapterAll = (chapterId: string, stepIds: string[], checked: boolean) => {
    ...
    if (checked) {
      // まだ達成していなかった章が、まとめて達成されたときにもフラッシュ
      setFlashChapters((current) => ({
        ...current,
        [chapterId]: true,
      }))
  ...
  return (
    <section
      ...
      className="space-y-8"
    >
      {INTERMEDIATE_CHAPTERS.map((chapter) => {
