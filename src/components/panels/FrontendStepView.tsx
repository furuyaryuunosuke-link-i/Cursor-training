import {
  getFrontendStepById,
  getFrontendPrevNext,
} from '../../data/frontendSteps'
import { GlassCard } from '../GlassCard'

type FrontendStepViewProps = {
  stepId: string
  onNavigateToStep: (stepId: string | null) => void
}

export function FrontendStepView({
  stepId,
  onNavigateToStep,
}: FrontendStepViewProps) {
  const resolved = getFrontendStepById(stepId)
  const { prev, next } = getFrontendPrevNext(stepId)

  if (!resolved) {
    return (
      <section
        id="panel-frontend-step"
        role="tabpanel"
