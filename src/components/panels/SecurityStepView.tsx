import {
  getSecurityStepById,
  getSecurityPrevNext,
} from '../../data/securitySteps'
import { GlassCard } from '../GlassCard'
import { StepNavBar } from '../StepNavBar'

type SecurityStepViewProps = {
  stepId: string
  onNavigateToStep: (stepId: string | null) => void
}

export function SecurityStepView({
  stepId,
  onNavigateToStep,
}: SecurityStepViewProps) {
  const resolved = getSecurityStepById(stepId)
  const { prev, next } = getSecurityPrevNext(stepId)

  if (!resolved) {
    return (
      <section
        id="panel-security-step"
        role="tabpanel"
