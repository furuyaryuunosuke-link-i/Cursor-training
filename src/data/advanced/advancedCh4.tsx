...
この関数に対して、pytest で「特定の row を渡したときに、期待どおりの dict が返る」テストを 1 本書いてください。テストファイルの置き場所と、実行コマンドも教えてください。`,
    ...
    step(
      ...
      <div className="space-y-4">
        ...
        <p>ここでは、既存のログ出力を確認し、自分で 1 か所「ここまで到達した」というログを追加して、実行後にログファイルで追えるようにします。</p>
        <h3>なぜこのステップが必要か</h3>
        <p>
          本番に近い環境では、画面で見えない部分で止まることが多いです。
          ログの設計（どこに何を書くか）を意識しておくと、トラブル時に「ログのこの行まで出ているから、この処理の前で止まっている」と切り分けやすくなります。
        </p>
      </div>,
      {
        samplePrompt: `Python のツールで、処理の開始・終了・エラー時にログを出すようにしたいです。

logging <GlossaryTerm name="module">モジュール</GlossaryTerm>で、ファイル（tool.log）とコンソールの両方に出力し、エラー時にはスタックトレースも含めて記録する設定の例を教えてください。`,
      }
    ),
    step(
      '4-4',
      ...
      <div className="space-y-4">
        <p>
          ...
          <strong>チェック項目（設定ファイルが読めるか、認証が有効か、必要なファイルがあるか）を実行して結果をログや <GlossaryTerm name="json">JSON</GlossaryTerm> に残す</strong>仕組みを、設計レベルで考えます。
        </p>
        <p>既存ツールにそのような仕組みがあれば、どこに書かれているかを確認します。</p>
        <h3>なぜこのステップが必要か</h3>
        <p>
          運用し始めると、「動かなくなったが、設定ミスなのかコードの不具合なのか分からない」という状況が出てきます。
          自己診断の項目を決めておくと、そのログを見るだけで「何が不足しているか」を判断しやすくなり、引き継ぎや障害対応が楽になります。
        </p>
      </div>,
      {
