    ...
    steps: [
      step(
        ...
        <div className="space-y-4">
          <p>
            まずは、自分の担当業務のなかで
            <strong>「どんな表作業をPythonで置き換えたいか」</strong>を具体的に書き出します。
          </p>
          <ul>
...
status が「面談済み」の候補者だけを print するコードも付けてください。`,
      ...
      step(
        ...
        'CSVから一覧を読み込んでフィルタする',
        <div className="space-y-4">
          <p>サンプルの候補者CSVを用意し、Pythonで読み込んで条件に合う行だけを取り出します。</p>
          <ul>
            <li>
              <GlossaryTerm name="csv">csv</GlossaryTerm> モジュールや{' '}
          ...
          <p>
            ...
            ここを押さえておくと、「手元の表ファイルをPythonで読み取って条件抽出する」という
      ...
      step(
        ...
        <div className="space-y-4">
          ...
          <p>
            ...
            後の章で Python 自体に悩む時間を減らせます。
            もしつまずいた部分があれば、その箇所をメモしておき、
            <GlossaryTerm name="cursor">Cursor</GlossaryTerm> の Ask モードで相談材料にしましょう。
          </p>
        </div>,
        {
    ...
    steps: [
      step(
        ...
        <div className="space-y-4">
          <p>
            まずは、これから作るツールの中で
            <strong>「コードに直接書かず、設定やファイルに分けておきたい情報」</strong>
            を洗い出します。
          </p>
          <ul>
          ...
          <p>
            ...
            先に「設定として外に出すもの」と「コードに残すもの」を切り分けておくことで、
      ...
      step(
        ...
        <div className="space-y-4">
          ...
          <p>
            ...
            に書き、Python から読み取る練習をします。
          </p>
          <h3>なぜこのステップが必要か</h3>
          <p>
            コードの中にIDやパスワードを書き込んでしまうと、共有や管理が危険になります。
            <GlossaryTerm name="envFile">.env</GlossaryTerm>
            や
            <GlossaryTerm name="environmentVariable">環境変数</GlossaryTerm>
            を使えるようになると、
            「コードは共有しても、機密情報は手元にだけ置く」という安全な運用がしやすくなります。
          </p>
        </div>,
        {
...
ライブラリを使う場合は、初心者向けにインストール方法と import の書き方も教えてください。`,
      ...
      step(
        ...
        <div className="space-y-4">
          ...
          <p>
            ここでは、「元の列名 → 変換後の列名」の対応表を YAML で書き、
            Python から読み取って変換に使うイメージをつかみます。
          </p>
          <h3>なぜこのステップが必要か</h3>
          <p>
            列名やシート名が変わるたびに
            ...
            <GlossaryTerm name="yaml">YAML</GlossaryTerm>
            でマッピングを定義しておくと、「設定だけ差し替えて同じツールを別案件でも使う」といった再利用がしやすくなります。
          </p>
        </div>,
        {
      ...
      step(
        ...
        <div className="space-y-4">
          <p>
            ...
            モジュールを使って、処理の開始・終了・エラー内容をログファイルに残す練習をします。
          </p>
          <p>
            ここでは、CSV 読み込みと書き出しの前後でメッセージを出し、エラー時には例外の情報をログに書くようにします。
          </p>
          <h3>なぜこのステップが必要か</h3>
          <p>
...
シンプルなサンプルコードを教えてください。`,
      ...
      step(
        ...
        <div className="space-y-4">
          ...
          <p>
            ここでは、処理をどんな関数に分けるか、どの情報を設定に寄せるかを文章で整理し、それをもとに AI
            に実装を手伝ってもらう準備をします。
          </p>
          <h3>なぜこのステップが必要か</h3>
          <p>
            設定・機密情報・ログの責務を分けておくと、規模が大きくなっても破綻しにくくなります。
            中級の時点で「設定とコードを分ける」意識を持っておくと、上級で複数機能を束ねたツールを作るときにもスムーズに応用できます。
          </p>
        </div>,
        {
      ...
      step(
        ...
        <div className="space-y-4">
          ...
          <p>
            ...
            と聞くと難しく感じますが、中身は「人がクリックしている手順をそのままコードに落とす」イメージです。
...
「自動化した方がよい作業」「人がやった方がよい作業」の考え方を教えてください。`,
      ...
      step(
        ...
        <div className="space-y-4">
          ...
          <h3>なぜこのステップが必要か</h3>
          <p>
            ブラウザ自動化コードは、「どのボタンを押すか」「どの入力欄に文字を入れるか」を
      ...
      step(
        ...
        <div className="space-y-4">
          <p>
            ...
            などのライブラリを使って、ブラウザを起動し、ログインフォームに自動で入力してログインするコードを書いてみます。
          </p>
          <p>
            ユーザー名・パスワードは
            <GlossaryTerm name="envFile">.env</GlossaryTerm>
...
開発者ツールで調べた id / name / class を差し替えれば使えるような形で、サンプルコードをお願いしたいです。`,
      ...
      step(
        ...
        <div className="space-y-4">
          ...
          <h3>なぜこのステップが必要か</h3>
          <p>
            ブラウザ自動化は、画面の変更や通信のタイミングに影響を受けやすい分野です。
...
例を交えながら教えてください。`,
      ...
      step(
        ...
        <div className="space-y-4">
          ...
          <ul>
            <li>入力ファイル（CSV / Excel）の形式や保管場所</li>
            <li>ブラウザ上で参照する情報</li>
            <li>出力ファイルや更新したい画面</li>
          </ul>
          <h3>なぜこのステップが必要か</h3>
          <p>
            入力と出力があいまいなままだと、コードを書いても「結局何ができたのか」が分かりにくくなります。
      ...
      step(
        ...
        <div className="space-y-4">
          <p>
            ここまで整理した情報をもとに、
            <GlossaryTerm name="cursor">Cursor</GlossaryTerm>
            の Ask モードで全体フローの設計相談をし、Agent モードでコードを生成してもらいます。
          </p>
          <p>
            一度で完璧なコードを求めるのではなく、
...
そのあと、Agent モードで少しずつコードを書いていきたいです。`,
      ...
      step(
        ...
        <div className="space-y-4">
          ...
          <ul>
            <li>ログにどんなメッセージが出ていれば安心か</li>
            <li>ブラウザや出力ファイルのどこを見て確認するか</li>
